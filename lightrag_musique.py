"""
LightRAG generality experiment — MuSiQue.
...
Stages:
    1. Index MuSiQue corpus (7,674 docs) with LightRAG
...
# Sharding config — split corpus into smaller indexes to avoid O(n) merge bottleneck
NUM_SHARDS = 8
...
# ── Custom LLM/embedding wrappers (same as smoke test) ───────────
...
def compress_lightrag_context(question, context, max_hops=3, budget_tokens=4000):
    ...
    Parses the context, runs BFS from question entities, and reassembles
    a compressed context organized by hop level.
...
async def index_corpus():
    """Index all MuSiQue documents with LightRAG."""
    ...
    for start in range(0, len(docs_text), BATCH_SIZE):
        ...
        print(f"  Indexed {done}/{len(docs_text)} ({elapsed:.0f}s)")
...
def print_summary(results):
    print("\n" + "=" * 60)
    print("LightRAG Generality Experiment — MuSiQue (70B)")
    print("=" * 60)
