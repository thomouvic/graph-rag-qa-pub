...
def find_top_k_cooccurrence_pairs(text_units: list[TextUnit], k: int, bow_threshold: int = 2000) -> list[tuple[str, str]]:
    ...
    # Step 2: Calculate co-occurrence, keeping only top-k per text unit via heaps
    import heapq
    keys = list(processed_texts.keys())
    # Min-heaps of size k: heap[key] = [(count, other_key), ...]
    heaps = {key: [] for key in keys}

    for i in range(len(keys)):
        for j in range(i + 1, len(keys)):
            cooccurrence_count = len(processed_texts[keys[i]] & processed_texts[keys[j]])
            if cooccurrence_count == 0:
                continue
            for src, dst in ((keys[i], keys[j]), (keys[j], keys[i])):
                h = heaps[src]
                if len(h) < k:
                    heapq.heappush(h, (cooccurrence_count, dst))
                elif cooccurrence_count > h[0][0]:
                    heapq.heapreplace(h, (cooccurrence_count, dst))

    # Step 3: Collect top-k pairs from each heap
    top_k_pairs = []
    for key, h in heaps.items():
        for count, other_key in h:
